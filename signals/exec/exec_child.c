...
/*   Updated: 2026/06/17 22:38:56 by hbelleuv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

static void	exec_with_slash(t_shell *shell, t_cmd *cmd)
{
...
static void	setup_child_fds(t_cmd *cmd, int relay_fd, int pipe_fd[2])
{
	if (relay_fd != -1)
	{
...
static void	resolve_path(t_shell *shell, t_cmd *cmd)
{
	if (ft_strchr(cmd->args[0], '/') != NULL)
		exec_with_slash(shell, cmd);
	else
	{
...
void	exec_child(t_shell *shell, t_cmd *cmd, int relay_fd, int pipe_fd[2])
{
	...
	setup_child_signal();
	setup_child_fds(cmd, relay_fd, pipe_fd);
