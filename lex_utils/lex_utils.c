...
ft_flush(t_struct_lex *lex)
{
    ...
    if (!value)
        return (1);
    new_node = ft_new_node(value, TOK_WORD);
    if (!new_node)
        return (1);
    if (ft_add_back(&lex->head_lst, new_node))
        return(1);
...
ft_append(t_struct_lex *lex)
{
    ...
    if (ft_flush(lex))
        return(1);
    lex->start = -1; 
    new_node = ft_new_node(">>", TOK_APPEND);
    if (!new_node)
        return (1);
    if (ft_add_back(&lex->head_lst, new_node))
        return (1);
...
ft_heredoc(t_struct_lex *lex)
{
    ...
    if (ft_flush(lex))
        return(1);
    lex->start = -1; 
    new_node = ft_new_node("<<", TOK_HERE_DOC);
    if (!new_node)
        return (1);
    if (ft_add_back(&lex->head_lst, new_node))
        return (1);
...
ft_redir_out(t_struct_lex *lex)
{
    ...
    if (ft_flush(lex))
        return (1);
    lex->start = -1; 
    new_node = ft_new_node(">", TOK_REDIR_OUT);
    if (!new_node)
        return (1);
    if (ft_add_back(&lex->head_lst, new_node))
        return (1);
...
ft_redir_in(t_struct_lex *lex)
{
    ...
    if (ft_flush(lex))
        return(1);
    lex->start = -1; 
    new_node = ft_new_node("<", TOK_REDIR_IN);
    if (!new_node)
        return (1);
    if (ft_add_back(&lex->head_lst, new_node))
        return (1);
